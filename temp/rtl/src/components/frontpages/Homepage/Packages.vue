    ...
    <div class="pt-96 bg-surface">
        <v-container class="max-width-1218 package py-0" id="pricing">
            <div class="pb-12 text-center">
                    ...
                    <p class="text-muted pt-4 text-17 lh-32">Our robust analytics offer rich insights into the information buyers want, informing where teams.</p>
                ...
                <v-col cols="12" lg="3" sm="6" v-for="card in Packages" :key="card.caption">
                    <v-card elevation="0" class="rounded-16 pa-6 border">
                        <div class="pb-4">
                            ...
                            <p class="text-14 text-muted font-weight-medium">{{ card.subtext }}</p>
                        </div>


                        <div class="d-flex align-center mt-3">
                            <h2 class="display-2 text-primary font-weight-bold">${{ card.price }}</h2>
                            <span class="text-muted mt-4 ms-2 text-16">/{{ card.period }}</span>
                        </div>
                        ...
                        <v-btn color="darkgray" size="large" class="mt-4 font-weight-medium" :href="card.url" target="_blank" flat block>{{
                            card.buttontext
                        }}</v-btn>
                    </v-card>
                </v-col>
